 ...
 *
 */
up.compiler('.bookings-grid-cards', function(gridEl) {
	var wrapperEl = gridEl.closest('#bookings_grid') || gridEl;
	...
	function setMultiSelect(enabled) {
		...
		for (var i = 0; i < items.length; i++) {
			var item = items[i];
			var shouldShow = item.getAttribute('data-multi') === state;
	...
	gridEl.addEventListener('toggle_ms', function() {
		setMultiSelect(gridEl.getAttribute('data-multi-select') !== 'true');
	});
});

up.compiler('.cps-bookings-view-switcher', function(switcherEl) {
	var wrapperEl = switcherEl.closest('#bookings_grid') || switcherEl;
	var tabs = switcherEl.querySelectorAll('[data-bookings-tab]');
	var panels = switcherEl.querySelectorAll('[data-bookings-panel]');
	var multiSelectControllers = wrapperEl.querySelectorAll('.multi-select-controller');

	function setView(view) {
		for (var i = 0; i < tabs.length; i++) {
			var tab = tabs[i];
			var isActive = tab.getAttribute('data-bookings-tab') === view;
		...
		for (var j = 0; j < panels.length; j++) {
			var panel = panels[j];
			panel.hidden = panel.getAttribute('data-bookings-panel') !== view;
		}

		for (var k = 0; k < multiSelectControllers.length; k++) {
			multiSelectControllers[k].hidden = view !== 'rooms';
		}
	}

	up.on(switcherEl, 'click', '[data-bookings-tab]', function(event, tab) {
		setView(tab.getAttribute('data-bookings-tab'));
	});
