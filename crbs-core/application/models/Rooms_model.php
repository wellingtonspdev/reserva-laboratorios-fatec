...
class Rooms_model extends CI_Model
{
	...
	public function room_info($room)
	{
		...
		foreach ($fields as $field) {
			...
			$field_value = NULL;
			$field_value_html = false;

			switch ($field->type) {
				case 'TEXT':
					$field_value = $field_values[$field->field_id] ?? '';
					$field_value_html = false;
				break;
				...
				case 'CHECKBOX':
					...
					if ($val) {
						$icon = 'check';
						$alt = html_escape(lang('app.yes'));
					} else {
						$icon = 'x';
						$alt = html_escape(lang('app.no'));
					}
					$field_value = cps_icon($icon, 'w-4 h-4', $alt) . " <span style='line-height:16px'>{$alt}</span>";
					$field_value_html = true;
				break;
			...
			$info[] = [
				...
				'label' => $field->name,
				'value' => $field_value,
				'value_html' => $field_value_html ?? false,
			];
