...
if ($room_count > 0) {
	...
	$this->table->set_template([
		'table_open' => '<table
			class="border-table table-responsive-cards"
			' . $table_attrs . '
	...
	foreach ($rooms as $room) {
		...
		$sort_img = cps_icon('chevron-down', 'w-4 h-4', 'sort');
		...
		if (!empty($room->photo) && $image_url = image_url($room->photo)) {
			...
			$icon_el = cps_icon('image', 'w-4 h-4', lang('room.field.photo'));
		...
		$this->table->add_row([
			['data' => ($room_count > 1) ? $sort_html : null, 'data-label' => ''],
			['data' => $name_html, 'data-label' => lang('room.field.name')],
			['data' => $location_html, 'data-label' => lang('room.field.location')],
			['data' => $owner_html, 'data-label' => lang('room.field.user_id')],
			['data' => $photo_html, 'data-label' => lang('room.field.photo')],
			['data' => $actions_html, 'data-label' => lang('app.actions')],
		]);
	}
}
