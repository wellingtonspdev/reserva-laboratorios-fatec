...
echo iconbar([
	array('setup', lang('app.action.back'), 'arrow_turn_left.png'),
...
>
	...
	<tbody>
		...
		foreach ($weeks as $week) {
			...
			$dot = week_dot($week);
			echo "<td style='text-align:center' data-label='" . lang('week.field.colour') . "'>{$dot}</td>";

			$name = html_escape($week->name);
			$link = anchor('weeks/edit/'.$week->week_id, $name);
			echo "<td data-label='" . lang('week.field.name') . "'>{$link}</td>";
