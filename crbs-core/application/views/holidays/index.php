...
>
	...
	<tbody>
		...
		foreach ($holidays as $holiday) {
			...
			echo "<td data-label=\"".lang('holiday.field.name')."\">{$link}</td>";

			$start = $holiday->date_start ? date_output_long($holiday->date_start) : '';
			echo "<td data-label=\"".lang('holiday.field.date_start')."\">{$start}</td>";

			$end = $holiday->date_end ? date_output_long($holiday->date_end) : '';
			echo "<td data-label=\"".lang('holiday.field.date_end')."\">{$end}</td>";

			// Duration
			$duration = 1 + ($holiday->date_start->diff($holiday->date_end)->format('%a'));
			$duration_str = sprintf('%s %s', $duration, strtolower(lang('cal.days')));
			echo "<td data-label=\"".lang('holiday.field.duration')."\">{$duration_str}</td>";

			echo "<td data-label=\"".lang('app.actions')."\">";
