...
class Atualizar extends MY_Controller
{
	...
	public function check()
	{
		...
		$this->data['title'] = 'Atualizar classroombookings';
	...
	private function check_requirements()
	{
		...
		if ( ! $res) {
			$_SESSION['requirements']['database'] = array('status' => 'err', 'message' => 'Nao foi possivel conectar com as configuracoes informadas.');
		} else {
		...
		if (count($missing_tables) > 0) {
			...
			$_SESSION['requirements']['database_has_tables'] = array(
				'status' => 'err',
				'message' => "O banco de dados nao possui as seguintes tabelas: {$missing_str}",
			);
		} else {
	...
	private function get_db_config()
	{
		...
		if ( ! is_file($file_path)) {
			show_error("O arquivo de configuracao database.php nao foi encontrado.");
		}

		require($file_path);

		if ( ! isset($db) || ! is_array($db)) {
			show_error("O arquivo database.php nao pode ser carregado ou nao esta no formato correto.");
		}

		// Set DB config from existing arquivo.
	...
	private function start_upgrade()
	{
		...
		if ( ! $this->copy_images()) {
			log_message("error", "Atualizar: could not copy old images.");
			return FALSE;
		}

		if ( ! $this->do_migrations()) {
			log_message("error", "Atualizar: unable to do database migrations.");
			return FALSE;
		}

		if ( ! $this->install_config()) {
			log_message("error", "Atualizar: Setting up new config failed.");
			return FALSE;
		}
	...
	/**
	 * Find room photos & school logo arquivos to copy.
	 *
	 */
	private function copy_images()
	{
		...
		foreach ($dirs as $type => $dir) {
			if ( ! is_dir($dir)) {
				log_message("info", "Atualizar: Images: {$type} dir '{$dir}' does not exist.");
				continue;
			}

			$files = directory_map($dir, 1);
			if (empty($files)) {
				log_message("info", "Atualizar: Images: {$type} dir does not have any arquivos.");
				continue;
			}
		...
		foreach ($sources as $source) {
			...
			if ( ! copy($source, $dest)) {
				log_message("info", "Atualizar: Images: Unable to copy $source => $dest.");
				$errors[] = $source;
			} else {
				log_message("info", "Atualizar: Images: Copied $source => $dest.");
			}
		}
	...
	private function do_migrations()
	{
		...
		if ( ! $this->migration->latest()) {
			log_message("error", "Atualizar: Migrations: Could not migrate to latest.");
			return FALSE;
		}
