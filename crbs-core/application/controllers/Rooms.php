...
class Rooms extends MY_Controller
{
	...
	function info($room_id)
	{
		...
		$this->data['return_uri'] = $this->room_info_return_uri();
		$this->data['room_info_embedded'] = up_target();

		$this->data['title'] = $room->name;
		$this->data['showtitle'] = $room->name;
	...
	private function room_info_return_uri(): string
	{
		$return_uri = $this->input->get('return_uri', TRUE);
		...
		if (empty($return_uri)) {
			...
			if (!empty($referer) && strpos($referer, $base_url) === 0) {
				$return_uri = substr($referer, strlen($base_url));
			}
		}

		if (empty($return_uri) || preg_match('#^https?://#i', $return_uri)) {
			$return_uri = 'bookings';
		}

		return ltrim($return_uri, '/');
	}
