	...
	*/
	public function ldap()
	{
		...
		if ($ldap_available) {
			...
			$form = $this->load->view('settings/authentication/ldap', $this->data, TRUE);
			$test = $this->load->view('settings/authentication/ldap_test', $this->data, TRUE);
			$body = $this->session->flashdata('saved');
			$body .= $form . $test;

		} else {
			$body = "<div class='cps-admin-card border-red-300 bg-red-50 text-cps-red'>" . msgbox('error', lang('auth.ldap.error.no_module')) . "</div>";
		}

		$back = anchor('setup', cps_icon('chevron-left', 'w-4 h-4') . lang('app.action.back'), ['class' => 'cps-admin-btn']);
		$this->data['body'] = "<div class='cps-admin-page'><div class='cps-admin-toolbar'><div>{$back}</div><div></div></div>{$body}</div>";
