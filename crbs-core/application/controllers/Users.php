...
class Users extends MY_Controller
{
	...
	function add()
	{
		...
		$back = anchor('users', cps_icon('chevron-left', 'w-4 h-4') . lang('app.action.back'), ['class' => 'cps-admin-btn']);
	...
	function delete($id)
	{
		...
		$back = anchor('users', cps_icon('chevron-left', 'w-4 h-4') . lang('app.action.back'), ['class' => 'cps-admin-btn']);
		$title = "<div class='cps-admin-toolbar'><div>{$back}</div><div></div></div><h2>{$this->data['title']}</h2>";
		$this->data['body'] = "<div class='cps-admin-page'>{$title}<div class='cps-admin-card'>" . $this->load->view('partials/deleteconfirm', $this->data, TRUE) . "</div></div>";
	...
	public function import()
	{
		...
		$form = $this->load->view('users/import/stage1', $this->data, TRUE);
		$side = $this->load->view('users/import/stage1_side', $this->data, TRUE);
		$back = anchor('users', cps_icon('chevron-left', 'w-4 h-4') . lang('app.action.back'), ['class' => 'cps-admin-btn']);
		$this->data['body'] = "<div class='cps-admin-page'><div class='cps-admin-toolbar'><div>{$back}</div><div></div></div><div class='cps-admin-layout'><div>{$form}</div><aside class='cps-admin-side-panel'>{$side}</aside></div></div>";
