...
class Departments extends MY_Controller
{
	...
	function add()
	{
		...
		$this->data['showtitle'] = $this->data['title'];
		$form = $this->load->view('departments/departments_add', NULL, TRUE);
		$back = anchor('departments', cps_icon('chevron-left', 'w-4 h-4') . lang('app.action.back'), ['class' => 'cps-admin-btn']);
		$this->data['body'] = "<div class='cps-admin-page'><div class='cps-admin-toolbar'><div>{$back}</div><div></div></div>{$form}</div>";
	...
	function edit($department_id = NULL)
	{
		...
		$this->data['showtitle'] = $this->data['title'];
		$form = $this->load->view('departments/departments_add', $this->data, TRUE);
		$back = anchor('departments', cps_icon('chevron-left', 'w-4 h-4') . lang('app.action.back'), ['class' => 'cps-admin-btn']);
		$this->data['body'] = "<div class='cps-admin-page'><div class='cps-admin-toolbar'><div>{$back}</div><div></div></div>{$form}</div>";
	...
	function delete($id = NULL)
	{
		...
		$this->data['showtitle'] = $this->data['title'];
		$back = anchor('departments', cps_icon('chevron-left', 'w-4 h-4') . lang('app.action.back'), ['class' => 'cps-admin-btn']);
		$confirm = $this->load->view('partials/deleteconfirm', $this->data, TRUE);
		$this->data['body'] = "<div class='cps-admin-page'><div class='cps-admin-toolbar'><div>{$back}</div><div></div></div><div class='cps-admin-card'>{$confirm}</div></div>";
