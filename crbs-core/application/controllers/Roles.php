...
class Roles extends MY_Controller
{
	...
	public function add()
	{
		...
		$back = anchor('roles', cps_icon('chevron-left', 'w-4 h-4') . lang('app.action.back'), ['class' => 'cps-admin-btn']);

		$body = "<div class='cps-admin-page'><div class='cps-admin-toolbar'><div>{$back}</div><div></div></div><div class='cps-admin-layout'><div>{$form}</div><aside class='cps-admin-side-panel'>{$users}</aside></div></div>";
	...
	public function delete($id)
	{
		...
		$this->data['body'] = "<div class='cps-admin-page'>{$title}<div class='cps-admin-card'>{$body}</div></div>";
