...
class MY_Controller extends CI_Controller
{
	...
	private function init_lang()
	{
		...
		$enabled_langs = is_array($enabled_langs) ? $enabled_langs : [];
		$enabled_langs[] = 'english';
		$enabled_langs = array_values(array_unique($enabled_langs));
		...
		$idiom = null;
		foreach ([$cookie_lang, $session_lang, $setting_lang, $default, 'english'] as $candidate) {
			if ($candidate && in_array($candidate, $all_languages, true) && in_array($candidate, $enabled_langs, true)) {
				$idiom = $candidate;
				break;
			}
		}
		...
		if ( ! $this->load_language($idiom)) {
			log_message('error', "Idioma '{$idiom}' falhou ao carregar. Tentando fallback.");
			foreach ([$default, 'english'] as $fallback) {
				if ($fallback !== $idiom && in_array($fallback, $all_languages, true) && $this->load_language($fallback)) {
					$idiom = $fallback;
					break;
				}
			}
	...
	protected function load_language(string $idiom)
	{
		...
		foreach ($lang_files as $lang_file) {
			$app_path = APPPATH . "language/{$idiom}/{$lang_file}_lang.php";
			$system_path = BASEPATH . "language/{$idiom}/{$lang_file}_lang.php";
			if ( ! is_file($app_path) && ! is_file($system_path)) {
				log_message('error', "Arquivo de idioma ausente: {$app_path}");
				return false;
			}
		}
		...
		try {
			...
			log_message('error', "Erro ao carregar idioma '{$idiom}': {$e->getMessage()}");
			return false;
		}
