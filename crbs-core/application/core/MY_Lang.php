	...
	public function get_languages()
	{
		...
		if ($fp = @opendir($core_path)) {
			while (false !== ($entry = readdir($fp))) {
				...
				$app_lang_exists = is_file($core_path.DIRECTORY_SEPARATOR.$entry.DIRECTORY_SEPARATOR.'app_lang.php');
				if ($core_exists && $app_lang_exists) {
					$langs[] = $entry;
				}
			}
		}
		...
		foreach ($custom_paths as $path) {
			...
			while (false !== ($entry = readdir($fp))) {
				...
				if ($core_exists && $custom_exists && ! in_array($entry, $langs)) {
					$langs[] = $entry;
					continue;
				}
			}
