...
class Install extends MY_Controller
{
	...
	public function __construct()
	{
		...
		if (config_item('is_installed') &&
			...
			$this->session->set_flashdata('saved', msgbox("info", "O classroombookings ja esta instalado."));
			return redirect('');
		}
	...
	public function config()
	{
		...
		if (empty($drivers)) {
			$notices .= msgbox('error', "Sua configuracao do PHP nao possui drivers MySQL disponiveis.");
			$notices .= msgbox('error', "Instale ou habilite a extensao 'mysqli' ou 'pdo_mysql' (preferencial).");
		}
		...
		if (is_file(FCPATH . 'local/config.php') && ! empty($drivers)) {
			...
			if ( ! is_array($local_config)) {
				show_error("O arquivo 'local/config.php' nao esta em um formato reconhecido.");
			}
			...
			if ( ! $res) {
				$this->data['notice'] = msgbox('error', "Nao foi possivel conectar ao banco de dados usando os dados do arquivo local/config.php.");
			} else {
		...
		if ($this->input->post()) {
			...
			$this->form_validation->set_rules('hostname', 'Host', 'required');
			$this->form_validation->set_rules('port', 'Porta', 'required|is_natural_no_zero');
			$this->form_validation->set_rules('database', 'Banco de dados', 'required');
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$db_ok = false;
				$db_msg = "Nao foi possivel conectar ao banco de dados com os valores informados.";
				try {
					$db_conn = @$this->load->database($db_config, true);
					$db_ok = ($db_conn->initialize() !== false);
				} catch (Exception $e) {
					$db_msg = "Erro de banco de dados: " . $e->getMessage();
				}
		...
		$this->data['title'] = 'Instalar classroombookings - Configuracao';
	...
	public function info()
	{
		...
		if ($this->input->post()) {
			...
			$this->form_validation->set_rules('name', 'Nome da instituicao', 'required');
			$this->form_validation->set_rules('admin_username', 'Usuario', 'required');
			$this->form_validation->set_rules('admin_password', 'Senha', 'required');
		...
		$this->data['title'] = 'Instalar classroombookings - Informacoes';
		$this->data['showtitle'] = $this->data['title'];
	...
	public function checks()
	{
		...
		$this->data['title'] = 'Instalar classroombookings - Verificar requisitos';
		$this->data['showtitle'] = $this->data['title'];
	...
	private function check_requirements()
	{
		// PHP version
		//
		$_SESSION['requirements']['php_version'] = array('message' => 'Sua versao do PHP e ' . PHP_VERSION . '.');
		$has_php = (version_compare(PHP_VERSION, '8.2.0', '>='));
		if ( ! $has_php) {
		...
		if ( ! $has_gd) {
			$message = "Instale ou habilite o modulo 'php_gd' na configuracao do PHP.";
		}
		...
		if ( ! is_dir($local_path))
		{
			...
			{
				$message = "A pasta 'local' nao existe e nao pode ser criada.";
				$has_local = FALSE;
			}
		}
		...
		{
			$has_local = FALSE;
			$message = "A pasta 'local' nao tem permissao de escrita.";
		}
		...
		if ( ! is_dir($uploads_path))
		{
			...
			{
				$message = "A pasta 'uploads' nao existe e nao pode ser criada.";
				$has_uploads = FALSE;
			}
		}
		...
		{
			$has_uploads = FALSE;
			$message = "A pasta 'uploads' nao tem permissao de escrita.";
		}
	...
	private function install_config_file()
	{
		...
		if ( ! $res) {
			log_message("error", "Nao foi possivel salvar a configuracao no arquivo.");
			$this->errors['config'] = "Nao foi possivel salvar a configuracao no arquivo.";
		}
