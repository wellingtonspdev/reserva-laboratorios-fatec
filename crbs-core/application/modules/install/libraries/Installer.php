...
class Installer
{
	...
	public function set_db_connection()
	{
		...
		if (empty($user) || empty($pass) || empty($db)) {
			$this->errors[] = 'Nenhum dado de conexao com o banco foi informado.';
			return false;
		}
		...
		if ( ! $db_ok) {
			$this->errors[] = 'Nao foi possivel conectar ao servidor de banco de dados.';
			return false;
		}
	...
	public function is_installed()
	{
		if ( ! $this->db_connection) {
			$this->errors[] = 'Sem conexao com o banco de dados.';
			return null;
		}

		$is_installed = $this->db_connection->table_exists('users');
		if ($is_installed) {
			$this->errors[] = 'A tabela do banco de dados ja existe.';
		}
	...
	public function execute()
	{
		...
		if ( ! $has_name) {
			$this->errors[] = 'Nenhum nome informado.';
			return $out;
		}
		...
		if ( ! $has_user) {
			$this->errors[] = 'Nenhum usuario informado.';
			return $out;
		}
	...
	private function run_sql_file($filename)
	{
		...
		if ( ! is_file($filepath)) {
			$this->errors[] = sprintf('Arquivo SQL %s nao encontrado.', $filename);
			return false;
		}
	...
	private function install_migration()
	{
		...
		if ( ! is_file($filepath)) {
			$this->errors[] = 'Arquivo de migracao nao encontrado.';
			return false;
		}
	...
	private function create_user()
	{
		...
		if ( ! $res && ! is_numeric($id)) {
			$this->errors[] = 'Nao foi possivel criar o usuario.';
			return false;
		}
